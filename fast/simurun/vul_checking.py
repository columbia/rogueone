...
def traceback(G, vul_type, start_node=None):
    ...
    non_locals = G.get_calls_to_non_local()
    if G.new_trace_rule:
    ...
    for func_node in func_nodes:
        ...
        if func_name in expoit_func_list or (G.vul_type == 'data_flow' and func_node in non_locals):

            caller = func_node

            caller_attrs = G.get_node_attr(caller)
            if 'callees' in caller_attrs and len(caller_attrs['callees'].difference(G.benign_builtins)) == 0:
                continue
            caller = G.find_nearest_upper_CPG_node(caller)
            caller_list.append("{} called {}".format(caller, func_name))
            paths = G._dfs_upper_by_edge_type(caller, "OBJ_REACHES")
...
def do_vul_checking(G, rule_list, paths):
    """
    checking the vuleralbilities in the pathes

    Args:
        G: the graph object
        rule_list: a list of pairs, (rule_function, args of rule_functions)
        paths: the possible paths
    Returns:
