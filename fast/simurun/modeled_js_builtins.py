...
def setup_js_builtins(G: Graph):
    ...
    for func in G.builtin_constructors:
        G.benign_builtins.add(func)
...
def setup_string(G: Graph):
    ...
    G.named_builtin_map[string_prototype] = 'String.prototype'
    G.named_builtin_map[string_cons] = 'String'
    # built-in functions for regexp
    funcs = [
        ('match', string_prototype, None),
        ('matchAll', string_prototype, None),
        ('replace', string_prototype, string_p_replace),
        ('search', string_prototype, string_p_match),
        ('split', string_prototype, string_p_split),
        ('substr', string_prototype, string_p_substr),
        ('substring', string_prototype, string_p_substring),
        ('reverse', string_prototype, string_p_reverse),
        ('toLowerCase', string_prototype, string_p_to_lower_case),
        ('toUpperCase', string_prototype, string_p_to_upper_case),
        ('trim', string_prototype, string_p_trim),
        ('trimEnd', string_prototype, string_p_trim_end),
        ('trimStart', string_prototype, string_p_trim_start),
        ('charAt', string_prototype, string_p_char_at),
        ('slice', string_prototype, string_returning_func),
    ]
    for func_tuple in funcs:
        func_obj = G.add_blank_func_as_prop(*func_tuple)
        G.benign_builtins.add(func_obj)
        G.named_builtin_map[func_obj] = "String." + func_tuple[0]
...
def setup_number(G: Graph):
    ...
    G.named_builtin_map[number_cons] = 'Number'
    G.named_builtin_map[number_prototype] = 'Number.prototype'
...
def setup_array(G: Graph):
    ...
    G.named_builtin_map[array_cons] = 'Array'
    G.named_builtin_map[array_prototype] = 'Array.prototype'
    funcs = [
        ('isArray', array_cons, array_is_array),

        # built-in functions
        ('push', array_prototype, array_p_push),
        ('pop', array_prototype, array_p_pop),
        ('unshift', array_prototype, array_p_push),
        ('shift', array_prototype, array_p_shift), # broken
        ('join', array_prototype, array_p_join),
        ('forEach', array_prototype, array_p_for_each_value),
        ('keys', array_prototype, array_p_keys),
        ('values', array_prototype, array_p_values),
        ('entries', array_prototype, array_p_entries),
        ('splice', array_prototype, array_p_splice),
        ('slice', array_prototype, array_p_slice),
        ('filter', array_prototype, this_returning_func),
        ('map', array_prototype, array_p_map),
        ('reduce', array_prototype, array_p_reduce),
        ('concat', array_prototype, array_p_concat),
    ]
    for func_tuple in funcs:
        func_obj = G.add_blank_func_as_prop(*func_tuple)
        G.benign_builtins.add(func_obj)
        G.named_builtin_map[func_obj] = 'Array.' + func_tuple[0]
...
def setup_browser_builtins(G: Graph):
    G.add_blank_func_to_scope("Document", scope=G.BASE_SCOPE, python_func=this_returning_func)
    doc_obj = G.add_obj_to_scope(name='document', scope=G.BASE_SCOPE)
    G.set_node_attr(doc_obj, ('tainted', True))
    G.set_node_attr(doc_obj, ('trust_domain', ':sys:frontend:document'))

    benigns = []

    child_arr = G.add_obj_as_prop(prop_name='forms', parent_obj=doc_obj, js_type='array')
    G.add_obj_as_prop(prop_name=0, parent_obj=child_arr, tobe_added_obj=doc_obj)
    benigns.append(G.add_blank_func_as_prop('getElementById', doc_obj, this_returning_func))

    for f in benigns:
        G.benign_builtins.add(f)

def setup_buffer(G: Graph):
    buffer_obj = G.add_blank_func_to_scope("Buffer", scope=G.BASE_SCOPE, python_func=object_constructor)
    G.set_node_attr(buffer_obj, ('trust_domain', ':encoding'))
    benigns = []
    benigns.append(G.add_blank_func_as_prop("from", buffer_obj, object_constructor))
    benigns.append(G.add_blank_func_as_prop("alloc", buffer_obj, object_constructor))
    for f in benigns:
        G.benign_builtins.add(f)
...
def setup_object_and_function(G: Graph):
    ...
    G.named_builtin_map[object_cons] = 'Object.constructor'

    G.builtin_constructors.append(object_cons)
    G.object_cons = object_cons
    G.named_builtin_map[object_cons] = 'Object'
    ...
    G.named_builtin_map[object_prototype] = 'Object.prototype'
    ...
    G.named_builtin_map[function_cons] = 'Function'
    G.builtin_constructors.append(function_cons)
    G.function_cons = function_cons
    G.named_builtin_map[function_cons] = 'Function'
    G.set_node_attr(function_cons, ('trust_domain', ':dyn:both:Function'))
    ...
    G.named_builtin_map[function_prototype] = 'Function.prototype'
    ...
    benign_funcs.append(G.add_blank_func_as_prop('keys', object_cons, object_keys))
    benign_funcs.append(G.add_blank_func_as_prop('values', object_cons, object_values))
    benign_funcs.append(G.add_blank_func_as_prop('entries', object_cons, object_entries))
    benign_funcs.append(G.add_blank_func_as_prop('defineProperty', object_cons, blank_func))
    ...
    benign_funcs.append(G.add_obj_as_prop('getOwnPropertySymbols', parent_obj=object_cons, tobe_added_obj=G.false_obj))

    benign_funcs.append(G.add_blank_func_as_prop('toString', object_prototype, object_p_to_string_simp))
    benign_funcs.append(G.add_blank_func_as_prop('toLocaleString', object_prototype, object_p_to_string_simp))
    benign_funcs.append(G.add_blank_func_as_prop('valueOf', object_prototype, this_returning_func))
    benign_funcs.append(G.add_blank_func_as_prop('hasOwnProperty', object_prototype, object_p_has_own_property))
...
def setup_global_functions(G: Graph):
    ...
    for o in [decode_uri, decode_uri_component, encode_uri, encode_uri_component, escape, unescape]:
        G.set_node_attr(o, ('trust_domain', ':encoding'))
    ...
    G.set_node_attr(require, ('trust_domain', ':dyn:require'))
    G.set_node_attr(jseval, ('trust_domain', ':dyn:eval'))
    for func in [parse_int, parse_float, decode_uri, decode_uri_component, encode_uri, encode_uri_component,
                 escape, unescape, set_interval, set_timeout, clear_interval, clear_timeout, require]:
        G.benign_builtins.add(func)
...
def set_timeout_mtq(G: Graph, call_ast, extra, _, *args):
    # Do not use this function for now. It can cause infinite recursive calls.
    def timeout_task(self, G):
        nonlocal args, extra, call_ast
        for arg in args:
            filtered_objs = list(filter(lambda obj:
                G.get_node_attr(obj).get('type') == 'function', arg.obj_nodes))
            opgen.call_function(
                G, filtered_objs, extra=extra, call_ast=call_ast)
    G.microtask_queue.append(timeout_task)
    return NodeHandleResult()
...
def setup_global_objs(G: Graph):
    ...
    G.benign_builtins.add(G.add_blank_func_as_prop('log', console_obj, console_log))
    G.benign_builtins.add(G.add_blank_func_as_prop('error', console_obj, console_log))
    G.benign_builtins.add(G.add_blank_func_as_prop('warn', console_obj, console_log))

    process_obj = G.add_obj_to_scope(name='process', scope=G.BASE_SCOPE, js_type='object', value=wildcard)
    G.benign_builtins.add(G.add_blank_func_as_prop('exit', process_obj, blank_func))
    G.set_node_attr(process_obj, ('fake_arg', True))
    G.set_node_attr(process_obj, ('tainted', True))
    p_td = 'process'
    G.set_node_attr(process_obj, ('trust_domain', p_td))
    ...
    G.set_node_attr(argv, ('trust_domain', p_td + '.argv'))
    ...
    env = G.add_obj_as_prop(prop_name='env', parent_obj=process_obj, js_type='object', value=wildcard)
    G.set_node_attr(env, ('trust_domain', p_td + '.env'))

    G.named_builtin_map[process_obj] = 'process'
    # What is in process?
    # User data: argv and env
    # Static data: platform, arch,
    # functions: _rawDebug, setuid, setguid
...
def promise_p_then(G: Graph, call_ast, extra, this, on_fulfilled=NodeHandleResult(), on_rejected=NodeHandleResult()):
    ...
    def check_promise_status(self, G):
        nonlocal old_promises, on_fulfilled, on_rejected, extra, call_ast
        # print(f"Promise registered by {call_ast} {G.get_node_attr(call_ast)}")
        # print(f"Extra: {extra}")
        if extra and extra.promise_reg.count(call_ast) >= G.call_limit:
            # print(f"{call_ast} registers promises too many times, returning...")
            return
        ...
        for promise in old_promises:
            ...
            if fulfilled_with is not None: # the promise is possibly fulfilled
                result, _ = opgen.call_function(G, on_fulfilled.obj_nodes,
                    ...
                    extra=ExtraInfo(extra, promise_reg=extra.promise_reg+[call_ast]),
