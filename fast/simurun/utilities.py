...
class ExtraInfo:
    def __init__(self, original=None, **kwargs):
        ...
        if original is not None:
            ...
            self.in_arg_list = original.in_arg_list
            self.promise_reg = original.promise_reg
        ...
        if 'in_arg_list' in kwargs:
            self.class_obj = kwargs.get('in_arg_list')
        if 'promise_reg' in kwargs:
            self.promise_reg = kwargs.get('promise_reg')

    def __bool__(self):
        return bool(self.branches or (self.side is not None) or
            (self.parent_obj is not None) or (self.caller_ast is not None)
            or (self.switch_var is not None) or (self.in_arg_list is not None)
            or bool(self.promise_reg))
