...
class TraceRuleOld(TraceRuleInterface):
    ...
    def exist_func(self, func_names, path):
        ...
        Returns:
            checking result
        """
        called_func_list = self.called_func_list(path)
    ...
    def called_func_list(self, path: list[str]) -> set[str]:
        called_func_list = set()
        for node in path:
            children = self.graph.get_all_child_nodes(node)
            for child in children:
                cur_node = self.graph.get_node_attr(child)
                if 'type' in cur_node:
                    if cur_node['type'] == 'AST_CALL' or cur_node['type'] == 'AST_METHOD_CALL':
                        cur_func = self.graph.get_name_from_child(child, order=1, funcName=True)
                        called_func_list.add(cur_func)
        return called_func_list
