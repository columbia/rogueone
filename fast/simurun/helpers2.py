...
def check_condition(G: Graph, ast_node, extra: ExtraInfo):
    ...
    if node_type == 'AST_BINARY_OP':
        ...
        if op_type == 'BINARY_BOOL_OR':
            ...
            #     f'{handled_right.name}: {right_values}, {op_type}')
...
def add_contributes_to(G: Graph, sources, target, operation: str=None,
    ...
    if G.finished or G.time_limit_reached: return
    assert not isinstance(sources, (str, bytes))
    tainted = False
    random = get_random_hex()
    if type(target) == list:
        logger.error(f"Trying to contributes_to to a list.  {G.entry_file_path}")
        target = target[0]
...
def check_code_coverage_increase(G: Graph):
    if G.prev_code_coverage_time is None or \
             time.time() - G.prev_code_coverage_time > 15:
        covered_stmt = len(G.covered_stat)
        total_stmt = G.get_total_num_statements()
        # TODO: Why does esprima not find any statements for some packages? (deep_update)
        total_stmt = max(total_stmt, 1)
        cur_code_coverage = covered_stmt / total_stmt
        # if G.prev_code_coverage is not None and \
        #         cur_code_coverage <= G.prev_code_coverage:
        #     logger.error(
        #         "Code coverage is not increasing. Terminating analysis...")
        #     G.finished = True
        #     G.timed_out_by_code_coverage = True
        G.prev_code_coverage = cur_code_coverage
        G.prev_code_coverage_time = time.time()
