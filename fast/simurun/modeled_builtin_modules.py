...
def read_file_sync(G: Graph, call_ast, extra, _, path=NodeHandleResult(),
                   options=None):
    paths = path.values
    ...
    returned_values = []
    returned_objs = []
    used_obj_lists = [[o] for o in path.obj_nodes] + path.value_sources
    for i, path in enumerate(paths):
        if path is None:
            continue
        if path == wildcard:
            returned_obj = G.add_obj_node(call_ast, 'string', wildcard)
            returned_objs.append(returned_obj)
            add_contributes_to(G, used_obj_lists[i], returned_obj)
            continue
        ...
        #     returned_obj = G.add_obj_node(call_ast, 'string', content)
        #     returned_objs.append(returned_obj)
        #     add_contributes_to(G, used_obj_lists[i], returned_obj)
        #     logger.debug(f'Read file {path}, content: ' + re.sub(r'\n|\t', '', content))
        # except Exception as e:
        #     log_prefix = f'Read file {path} failed: {str(e)}, return a wildcard object'
        #     if default_exception(e, logging_prefix=log_prefix, rethrow_unexpected=False):
        #         raise e
        returned_obj = G.add_obj_node(call_ast, 'string', wildcard)
        returned_objs.append(returned_obj)
        add_contributes_to(G, used_obj_lists[i], returned_obj)
    ...
    return NodeHandleResult(obj_nodes=returned_objs,
                            used_objs=list(chain(*used_obj_lists)))
    ...
    for func_obj in func_objs:
        def gen_func(func_obj):
            def returned_func(G: Graph, caller_ast, extra, this, *args):
                def executor(G: Graph, caller_ast, extra, this, on_fulfilled=NodeHandleResult(),
