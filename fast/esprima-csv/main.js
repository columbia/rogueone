    ...
    */
    ...
    switch (currentNode.type) {
            ...
            if (outputStyle == 'c') {
                ...
                for (param of currentNode.params) {
                    function addParam(param){
                        if (!param) return; // mst-gql-0.14.1/generator/generate.js
                        // write the Parameter virtual node
                        nodeIdCounter++;
                        let vParameterId = nodeIdCounter;
                        relsStream.push([vNodeId, vParameterId, parentOf].join(delimiter) + '\n');
                        nodes[vParameterId] = {
                            ...
                            phpflag: phpflag,
                            childNum: vNodeChildNumberCounter,
                            code: (param ? param.name : null) || null, // mst-gql-0.14.1/generator/generate.js
                            lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                            funcId: currentFunctionId
                        };
        ...
        case "ImportDeclaration":
            for (let specifier of currentNode.specifiers){
                nodeIdCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(specifier, nodeIdCounter, currentId, childNumberCounter, currentFunctionId, null);
                childNumberCounter++;
            }
            nodeIdCounter++;
            relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
            dfs(currentNode.source, nodeIdCounter, currentId, childNumberCounter, currentFunctionId, null);
            nodes[currentId] = {
                label: 'AST',
                type: currentNode.type,
                phptype: 'JS_IMPORT_DECLARATION',
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                childNum: childNum,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
        ...
        case "ImportDefaultSpecifier":
            phptype = "JS_IMPORT_DEFAULT_SPECIFIER";
        case "ImportNamespaceSpecifier":
            phptype = "JS_IMPORT_NAMESPACE_SPECIFIER";
            nodeIdCounter++;
            relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
            dfs(currentNode.local, nodeIdCounter, currentId, 0, currentFunctionId, null);
            nodes[currentId] = {
                label: 'AST',
                type: currentNode.type,
                phptype: phptype,
        ...
        case "ImportSpecifier":
            ...
            nodeIdCounter++;
            relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
            dfs(currentNode.imported, nodeIdCounter, currentId, 1, currentFunctionId, null);
            nodes[currentId] = {
                label: 'AST',
                type: currentNode.type,
                phptype: 'AST_IMPORT_SPECIFIER',
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                childNum: childNum,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
...
function analyze(filePath, parentNodeId) {
    ...
    try {
        ...
        console.error(JSON.stringify({"ecmaVersion":options.ecmaVersion, "sourceType":options.sourceType}));
        if(sourceCode.includes("async function") || sourceCode.includes(" async ") || sourceCode.includes(" await ")){
            options.ecmaVersion = 8;
        }
        var root = esprima.parse(sourceCode, options);
